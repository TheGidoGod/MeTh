(() => {
  ...
  const PROBLEM_TYPES = [
    ...
    },
    {
      ...
      generate: (diffKey) => {
        ...
        const ans = base ** exp;
        const promptHTML = `Compute <span class="math">${base}<sup>${exp}</sup></span>.`;
        return {
          typeLabel: "Powers",
          promptHTML,
          hintHTML: `Multiply by the base repeatedly: ${base}^${exp} = ${base} \\times ... \\times ${base}.`,
          explanationHTML: `${base}<sup>${exp}</sup> = <span class="math">${ans}</span>.`,
          validate: (input) => {
            const n = parseNumberLoose(input);
            return n !== null && n === ans;
          },
    ...
    },
    {
      ...
      generate: (diffKey) => {
        ...
        return {
          ...
          promptHTML,
          hintHTML: `Move the decimal point two places: ${p}% = ${p}/100 = ${expected2}.`,
    ...
    },
    {
      ...
      generate: (diffKey) => {
        ...
        const hintHTML = `Distance = \\sqrt{(x2-x1)^2 + (y2-y1)^2}. Here those differences form a Pythagorean triple.`;
        const explanationHTML = `\\Delta x = ${Math.abs(x2 - x1)}, \\Delta y = ${Math.abs(y2 - y1)}. So distance = \\sqrt{${t.dx}^2 + ${t.dy}^2} = <span class="math">${t.h}</span>.`;
        return {
          typeLabel: "Distance",
          promptHTML,
          hintHTML,
    ...
    },
    {
      ...
      generate: (diffKey) => {
        ...
        const promptHTML = `Roll a fair ${sides}-sided die exactly <span class="math">${diceCount}</span> times. What is the expected total?`;
        const hint = `Expected value adds: E(total) = ${diceCount} \\times E(one die) = ${diceCount} \\times (${sides}+1)/2.`;
    ...
    },
    {
      ...
      generate: (diffKey) => {
        ...
        const promptHTML = askMedian
          ? `Find the <b>between</b> value of this set: <span class="math">{ ${nums.join(", ")} }</span>.`
          : `Find the <b>average</b> of this set: <span class="math">{ ${nums.join(", ")} }</span>. (You can answer as a decimal.)`;
        const hintHTML = askMedian
          ? `Between means the middle value when the numbers are sorted.`
          : `Average = (sum of values) / (number of values).`;
        const explanationHTML = askMedian
          ? `When sorted, the between value is <span class="math">${median}</span>.`
          : `Average = (sum) / ${nums.length} = <span class="math">${mean.toFixed(2)}</span>.`;
        return {
          typeLabel: askMedian ? "Between" : "Average",
          promptHTML,
          hintHTML,
          explanationHTML,
          validate: (input) => {
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (k === "h" && !els.hintBtn.disabled) onHint();
    if (k === "s" && !els.skipBtn.disabled) onSkip();
  });
